function [mseval,msevec] = fitS2EngBerScrambled(coordopt,modelopt,sigvec,sigm,plotopt)
...
for EB = [3 2 1 4] % 1 - Ber-GB; 2 - Ber-NW; 3 - Eng-GB; 4 - Eng-NW
    ...
    elseif EB == 1
        bound = loadBbnd(stimline,coordopt);
        cm = loadEmeans(stimline,coordopt); % Berinmo prototypes
        csig = sigvec{1};
        [pairWithin,pairAcross] = loadE2pairs(stimline,coordopt);
        tlabel = 'Berinmo (GB)';
    elseif EB == 4
        bound = loadEbnd(stimline,coordopt);
        cm = loadBmeans(stimline,coordopt); % English prototypes
        csig = sigvec{2};
        [pairWithin,pairAcross] = loadBpairs(stimline,coordopt);
        tlabel = 'English (NW)';
    end
