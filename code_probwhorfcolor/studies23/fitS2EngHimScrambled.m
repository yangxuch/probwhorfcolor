function [mseval,msevec] = fitS2EngHimScrambled(coordopt,modelopt,sigvec,sigm,plotopt)
...
for EH = [3 2 1 4] % 1 - Him-GB; 2 - Him-DH; 3 - Eng-GB; 4 - Eng-DH
    ...
    elseif EH == 1
        bound = loadHbnd(stimline,coordopt);
        cm = loadEmeans(stimline,coordopt); % Himba prototypes
        csig = sigvec{1};
        [pairWithin,pairAcross] = loadE2pairs(stimline,coordopt);
        tlabel = 'Himba (GB)';
    elseif EH == 4
        bound = loadEbnd(stimline,coordopt);
        cm = loadHmeans(stimline,coordopt); % English prototypes
        csig = sigvec{3};
        [pairWithin,pairAcross] = loadHpairs(stimline,coordopt);
        tlabel = 'English (DB)';
    end
